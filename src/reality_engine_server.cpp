...
private:
  ...
  static RealityEvent vector_from_json(const Json& body) {
    ...
    for (const auto& next : body.at("nextEventIds").is_array() ? body.at("nextEventIds").array() : Json::Array{}) vector.add_next_vector(next.as_string());
    for (const auto& output : body.at("outputEvents").is_array() ? body.at("outputEvents").array() : Json::Array{}) {
  ...
  static CriticalEventSequence sequence_from_json(const Json& body) {
    ...
    for (const auto& vector : body.at("events").is_array() ? body.at("events").array() : Json::Array{}) seq.add_vector(vector_from_json(vector));
    return seq;
  }
  ...
  http::Response demo_machine_response(const std::string& machineName, const std::string& fileName, const std::string& displayName) {
    ...
    if (inputSequences.is_array() && !inputSequences.array().empty()) {
      const Json& vectors = inputSequences.array().front().at("events");
