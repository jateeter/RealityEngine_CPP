...
echo "HealthKit Spezi bridge e2e"
echo "  Reality Engine port:    $REALITY_ENGINE_E2E_PORT"
echo "  Perception Engine port: $PERCEPTION_ENGINE_E2E_PORT"

bin/reality_engine_server "$REALITY_ENGINE_E2E_PORT" "$MACHINES_DIR" "$VECTOR_DIMENSION" >/tmp/reality_engine_healthkit_spezi_e2e.log 2>&1 &
REALITY_PID="$!"
...
bad_status="$(curl -s -o /tmp/healthkit_spezi_unauthorized.json -w "%{http_code}" -X POST "$PE_URL/api/integrations/healthkit/ingest" \
  -H "Content-Type: application/json" \
...
bad_bearer_status="$(curl -s -o /tmp/healthkit_spezi_bad_bearer.json -w "%{http_code}" -X POST "$PE_URL/api/integrations/healthkit/ingest" \
  -H "Content-Type: application/json" \
...
bearer_status="$(curl -s -o /tmp/healthkit_spezi_bearer.json -w "%{http_code}" -X POST "$PE_URL/api/integrations/healthkit/ingest" \
  -H "Content-Type: application/json" \
...
echo "HealthKit Spezi bridge e2e tests passed"
