# Bibliography

| Reference | Use in this repository |
| --- | --- |
| Boost.Asio documentation | Native asynchronous server/client transport. |
| Boost.Beast documentation | HTTP parsing, serialization, sessions, and client calls. |
| C++ standard library documentation | Threading, futures, synchronization, containers, and filesystem APIs. |
| OpenAPI Specification | Native service API contracts in `docs/openapi/`. |
| Qdrant documentation | Shared vector database verification and integration boundary. |
| RealityEngine_AI repository | Behavioral reference implementation and machine corpus. |
| localAIStack repository | Shared Qdrant/Ollama/RAG/agent integration surface. |
| Mermaid documentation | Diagrams embedded in Markdown docs. |

Project-local references:

| Document | Purpose |
| --- | --- |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Current native architecture. |
| [API_EQUIVALENCE.md](API_EQUIVALENCE.md) | C++ API status against the AI reference. |
| [VECTOR_MANAGEMENT.md](VECTOR_MANAGEMENT.md) | Dynamic vector management plan. |
| [E2E_TESTING.md](E2E_TESTING.md) | End-to-end validation of the machine corpus and service boundary. |
