# HealthKit Spezi Bridge — CPP Runtime

Runtime-specific connection guide for the C++ (Boost.Beast) Perception Engine.
...
```
POST http://<mac-lan-ip>:5300/api/integrations/healthkit/ingest
```

Port `5300` is the default CPP PE port (instance cpp-1). Additional instances
use `5400`, `5500`, etc. (`+100` per instance pair).

Do not use `localhost` from a simulator or iPhone unless the CPP PE runs on
the same device. For simulator-to-Mac, `http://127.0.0.1:5300` works. For a
...
```text
PE_BASE_URL=http://<mac-lan-ip>:5300
...
and downstream RE transitions against the CPP PE on port 5300.
